...
/* ============================================================
   Page logic — ROI calculator + application-form submit.
   (was an inline <script> at the bottom of each landing page)
   ============================================================ */
(function () {
  function init() {
    var $ = function (id) { return document.getElementById(id); };
    if (!$('cand') && !$('form')) return;               // not this page
    var zl = function (n) {
      return String(n).replace(/\B(?=(\d{3})+(?!\d))/g, ' ') + ' zł';
    };
    function calc() {
      var c = +$('cand').value, r = +$('rate').value, m = +$('mon').value;
      $('outCand').textContent = c; $('outRate').textContent = zl(r); $('outMon').textContent = m;
      $('outMonth').textContent = zl(c * r); $('outTotal').textContent = zl(c * r * m); $('outYear').textContent = zl(c * r * 12);
    }
    ['cand', 'rate', 'mon'].forEach(function (id) { $(id).addEventListener('input', calc); });
    calc();
    $('form').addEventListener('submit', function (e) {
      e.preventDefault();
      var f = e.target;
      if (!f.checkValidity()) { f.querySelectorAll(':invalid')[0].focus(); return; }
